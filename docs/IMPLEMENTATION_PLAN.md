...
- ✅ Parallelize scanning across actions (`--jobs`, default hardware concurrency): an `absl::Mutex`-guarded worker pool (fully thread-safety-annotated; `-Wthread-safety` + a Linux tsan CI job). The scan decision stays serial so the sidecar is deterministic.
